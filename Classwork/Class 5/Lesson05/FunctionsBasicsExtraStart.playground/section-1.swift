...
func funcKnockMultipleTimes(varTimesHint:Int) {
    for index in 1...varTimesHint {
        funcKnockKnock()
    }
}

funcKnockMultipleTimes(6)
...
func funcReplyWhoIsHere (varFirstName: String, varHomeCountry: String) {
    println("It's me, \(varFirstName), from \(varHomeCountry)")
}

funcReplyWhoIsHere("Tim", "America")
...
func funcCalculateAge (varYearOfBirth: Int) -> Int {
    let currentDate = NSDate()
    let currentCalendar = NSCalendar.currentCalendar()
    let currentYear = currentCalendar.component(NSCalendarUnit.CalendarUnitYear, fromDate: currentDate)
    let age = currentYear - varYearOfBirth
    return(age)
}

funcCalculateAge(1970)
...
func funcTellNameAndAge (varName: String, varYearOfBirth: Int) {
    let varAge = funcCalculateAge(varYearOfBirth)
    println("\(varName), my age is \(varAge)")
}

funcTellNameAndAge("Tim", 1970)
